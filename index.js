const navItems = ['Home', 'Service', 'About', 'Contact'];
  ...
  document.addEventListener('scroll', highlightActiveItem);
  window.addEventListener('load', highlightActiveItem);
...
schoolSection.innerHTML = SchoolsArr.map(school =>
  ...
  <div class='container'>
    ...
    <div class='right'>
   ${
    school.certificate ? `<h1>${school.certificate}</h1>` : ''
  }
...
workSection.innerHTML = WorksArr.map(work =>
  ...
  <div class='container'>
    ...
    <div class='right'>
   ${
    work.certificate ? `<h1>${work.certificate}</h1>` : ''
  }
    <p>${work.paragraph}</p>
    </div>
  </div>
  ...
  function validate() {
    ...
    if (nameField.value.trim() === "") {
      nameError.textContent = "Name field can't be blank.";
